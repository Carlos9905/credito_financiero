...
class SaleOrderLine(models.Model):
    ...
    lot_id = fields.Many2one("stock.production.lot", "IMEI", copy=False)
    venta_servicio = fields.Boolean("Es venta de servicio", default=False)
    ...
    def _onchange_product_id_set_lot_domain(self):
        if self.product_id.tracking == "serial" or self.product_id.tracking == "lot":
            self.venta_servicio = False
        else:
            self.venta_servicio = True
